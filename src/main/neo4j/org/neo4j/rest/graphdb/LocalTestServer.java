...
public class LocalTestServer {
	...
	public void start() {
		...
		if (url == null)
			throw new IllegalArgumentException(
					"Could not resolve properties file " + propertiesFile);
		final Jetty6WebServer jettyWebServer = new Jetty6WebServer();
		/*
		 * {
		 * 
		 * @Override protected void startJetty () { final Server jettyServer =
		 * getJetty (); jettyServer . setStopAtShutdown (true); final
		 * JettyStartupListener startupListener = new JettyStartupListener ();
		 * jettyServer . getServer (). addLifeCycleListener ( startupListener );
		 * // System .err .println( "jetty is started before notification " +
		 * jettyServer . isStarted ());
		 * 
		 * super. startJetty ();
		 * 
		 * startupListener .await(); jettyServer . removeLifeCycleListener (
		 * startupListener ); // System .err .println(
		 * "jetty is started after notification " + jettyServer . isStarted ());
		 * }
		 * 
		 * @Override public void stop() { final Server jettyServer = getJetty
		 * (); final JettyStartupListener listener = new JettyStartupListener
		 * (); jettyServer . getServer (). addLifeCycleListener ( listener) ;
		 * 
		 * super.stop ();
		 * 
		 * listener. await(); jettyServer . removeLifeCycleListener ( listener)
		 * ; } };
		 */
		...
		neoServer = new CommunityNeoServer(new PropertyFileConfigurator(
			...
			protected Iterable<ServerModule> createServerModules() {
				return null;
				// return asList(new
				// RESTApiModule(webServer,database,configurator.configuration()),new
				// ThirdPartyJAXRSModule(webServer,configurator));
			}
		};
		neoServer.start();
		try {
	...
	public void cleanDb() {
		Neo4jDatabaseCleaner cleaner = new Neo4jDatabaseCleaner(
				getGraphDatabase());
		cleaner.cleanDb();
	}
